...
print("The result weight(w0~wn) of Pocket is: " + str(w) + " with accuracy: " + str(accuracy*100) + "%")
plt.plot(np.arange(1,7000,1),record[1:7000],label='PLA',color='r')
plt.plot(np.arange(1,7000,1),record_best[1:7000],label='Pocket',color='b')
plt.legend()
plt.show()
w, record, cost = LogitRegression(data_clas,3,5)
print("The result weight(w0~wn) of Logistic Regression is: " + str(w) + " with accuracy: " + str(100-(record[-1]/20)) + "%")
...
for row in data_clas:
    if row[3] == 1:               # row[3] for question 1, row[4] for question 2 3
...
ax.scatter(dcla_ne[:,0],dcla_ne[:,1],dcla_ne[:,2], c='r',marker='_')
